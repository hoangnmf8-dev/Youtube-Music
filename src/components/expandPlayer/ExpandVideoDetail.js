...
import { calcVideoTime } from "../../utils/calcListTime";

function ExpandVideoDetail(data = []) {
  return `
    <div class="expand-video-detail basis-1/2 overflow-y-auto max-h-screen pr-2">
      <h3
      ...
      >
       ${data.map(item => `
        <div
          class="video-detail-item flex items-center gap-4 p-3 rounded-lg text-white hover:bg-white/10 cursor-pointer transition group"
        ...
        >
          ...
          <div class="text-sm text-white/50">${escapeHTML(calcVideoTime(item.duration))}</div>
        </div>
        `).join("")}
      </div>
    </div>
...
export default ExpandVideoDetail;