...
import calcListTime, { calcVideoTime } from "../utils/calcListTime";
    ...
    <div class="p-2">
      <div class="wrapper px-2">
        <div class="row flex flex-col flex-wrap justify-between lg:flex-row items-center lg:items-start">
          <div class="video-infor hidden sm:flex sm:static lg:sticky lg:top-24 text-white flex-col md:basis-1/4 lg:basis-1/2 gap-5 items-center w-[400px] shrink-0 grow-0">
          </div>

          <div class="video-detail w-full lg:basis-1/2">
          </div>
        </div>
      </div>
    </div>
...
const render = async (slug) => {
  const videoInforSection = $("#main .video-infor");
  const videoDetailSection = $("#main .video-detail");

  const videoDetailData = await getVideoDetail(slug);
  videoDetailData.related.forEach((item, index) => item._id = index);
  if(videoDetailData.related?.length) {
    sessionStorage.setItem("data_video", JSON.stringify(videoDetailData.related));
  }

  videoInforSection.innerHTML = `
    <p class="mb-4">Đang phát</p>
    <h1 class="title text-xl xl:text-3xl font-bold text-center">${escapeHTML(videoDetailData.title)}</h1>
    <p class="duration text-white/60 text-sm">Thời lượng: ${escapeHTML(calcListTime(videoDetailData.duration))}</p>
    <p class="views text-white/60 text-sm">${escapeHTML(calcViews(videoDetailData.popularity))} lượt xem</p>
  `;

  videoDetailSection.innerHTML = `
    ${videoDetailData.related.map(
        (item, index) => `
      <div data-id="${escapeHTML(index)}" data-src="${item.audioUrl}" class="video-detail-item flex w-full items-center px-3 py-4 gap-4 hover:bg-[#25384a] cursor-pointer transition-all duration-100 text-white group rounded-lg">
